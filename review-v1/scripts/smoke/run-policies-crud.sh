...
# 4. Update
UPDATED=$(api_post "/v1/policies/$PID" "$(echo '{"name":"crud-updated"}' | sed 's/POST/PATCH/')" 2>/dev/null || \
  curl -sf -X PATCH -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" -d '{"name":"crud-updated"}' "$API_BASE/v1/policies/$PID")
pass "Update"
