...
import (
	...
	"sim-engine/internal/config"
	"sim-engine/internal/model"
	"sim-engine/internal/sim"
...
func New(cfg config.Config, st *store.Store) *Server {
	s := &Server{cfg: cfg, store: st, mux: http.NewServeMux(), tr: otel.Tracer("sim-engine/http")}
	s.routes()
	return s
}
...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("/admin/run/enforcement", s.withAuth(s.handleIngestEnforcement))
...
func (s *Server) withAuth(next func(http.ResponseWriter, *http.Request)) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		tracer := s.tr
		if tracer == nil {
			tracer = otel.Tracer("sim-engine/http")
		}
		...
		if strings.TrimSpace(s.cfg.InternalKey) != "" {
			if r.Header.Get("X-Sim-Engine-Internal-Key") != s.cfg.InternalKey {
...
func (s *Server) handleWorldObserve(w http.ResponseWriter, r *http.Request) {
	...
	annotateSpan(r.Context(), req.OrgID, req.AgentID, req.RunID, req.StepID, "world.observe", requestID, "pending", nil)
	...
	annotateSpan(r.Context(), req.OrgID, req.AgentID, req.RunID, req.StepID, "world.observe", requestID, "allow", nil)
}
...
func (s *Server) handleWorldMove(w http.ResponseWriter, r *http.Request) {
	...
	requestID := requestIDFrom(r, req.RequestID)
	annotateSpan(r.Context(), req.OrgID, req.AgentID, req.RunID, req.StepID, "world.move", requestID, "pending", nil)
	if req.OrgID == "" || req.RunID == "" || req.AgentID == "" {
		s.writeFailure(w, requestID, http.StatusBadRequest, "INVALID_INPUT", "org_id, run_id, agent_id are required", false)
		return
	}
	...
	annotateSpan(r.Context(), req.OrgID, req.AgentID, req.RunID, req.StepID, "world.move", requestID, "allow", nil)
}
...
func (s *Server) handleCreateRun(w http.ResponseWriter, r *http.Request) {
	...
	annotateSpan(r.Context(), req.OrgID, "", req.RunID, 0, "world.run.create", requestIDFrom(r, ""), "pending", nil)
	if req.OrgID == "" {
	...
	annotateSpan(r.Context(), req.OrgID, "", runID, 0, "world.run.create", requestID, "allow", nil)
}
...
type enforcementEventRequest struct {
	OrgID     string `json:"org_id"`
	RunID     string `json:"run_id"`
	StepID    int64  `json:"step_id"`
	AgentID   string `json:"agent_id"`
...
func (s *Server) handleReplayRun(w http.ResponseWriter, r *http.Request) {
	...
	annotateSpan(r.Context(), req.OrgID, "", req.RunID, 0, "world.run.replay", requestIDFrom(r, ""), "pending", nil)
	...
	annotateSpan(r.Context(), req.OrgID, "", req.RunID, 0, "world.run.replay", requestID, "allow", nil)
}

func (s *Server) handleIngestEnforcement(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": map[string]any{"code": "INVALID_INPUT", "message": "method not allowed"}})
		return
	}
	var req enforcementEventRequest
	if err := decodeJSON(r, &req); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": map[string]any{"code": "INVALID_INPUT", "message": "invalid json"}})
		return
	}
	...
	annotateSpan(r.Context(), req.OrgID, strings.TrimSpace(req.AgentID), req.RunID, req.StepID, strings.TrimSpace(req.ToolName), requestID, "deny", policyIDPtr)
	if req.OrgID == "" || req.RunID == "" || req.EventType == "" {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": map[string]any{"code": "INVALID_INPUT", "message": "org_id, run_id, event_type are required"}})
		return
	}
	if req.StepID < 0 {
	...
	if req.EventType != "tool.denied" && req.EventType != "tool.rate_limited" {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": map[string]any{"code": "INVALID_INPUT", "message": "event_type must be tool.denied or tool.rate_limited"}})
		return
	}
	if _, err := s.store.GetRun(r.Context(), req.OrgID, req.RunID); err != nil {
		writeJSON(w, http.StatusNotFound, map[string]any{"error": map[string]any{"code": "NOT_FOUND", "message": "run not found"}})
		return
	}
	seq, err := s.store.NextSeq(r.Context(), req.RunID)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": map[string]any{"code": "PROVIDER_ERROR", "message": "failed to allocate sequence"}})
		return
	}
	...
	payload := baseEventPayload(req.EventType, req.OrgID, req.RunID, req.StepID, seq, strings.TrimSpace(req.AgentID), toolName, requestID)
	if policyID != "" {
		payload["policy_id"] = policyID
	}
	if strings.TrimSpace(req.Detail) != "" {
		payload["detail"] = strings.TrimSpace(req.Detail)
	}
	if strings.TrimSpace(req.Bucket) != "" {
		payload["bucket"] = strings.TrimSpace(req.Bucket)
	}
	if req.Limit > 0 {
		payload["limit"] = req.Limit
	}
	if err := s.store.InsertEvent(r.Context(), model.Event{
		RunID:     req.RunID,
		StepID:    req.StepID,
		Seq:       seq,
		OrgID:     req.OrgID,
		AgentID:   strings.TrimSpace(req.AgentID),
		ToolName:  req.EventType,
		Payload:   mustJSON(payload),
		RequestID: requestID,
	}); err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": map[string]any{"code": "PROVIDER_ERROR", "message": "failed to persist event"}})
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"run_id": req.RunID, "seq": seq, "event_type": req.EventType, "request_id": requestID})
}
...
func annotateSpan(ctx context.Context, orgID, agentID, runID string, stepID int64, toolName, requestID, decision string, policyID *string) {
	span := trace.SpanFromContext(ctx)
	if !span.SpanContext().IsValid() {
		return
	}
	attrs := []attribute.KeyValue{
		attribute.String("nexus.org_id", orgID),
	...
	if policyID != nil && strings.TrimSpace(*policyID) != "" {
		attrs = append(attrs, attribute.String("nexus.policy_id", strings.TrimSpace(*policyID)))
	}
