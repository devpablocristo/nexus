...
func ApplyMove(st *RuntimeState, agentID string, intent MoveIntent) MoveResult {
	...
	for _, id := range agentIDs {
		other := st.Agents[id]
		d := dist(candX, candY, other.X, other.Y)
		if d >= 2*st.Config.AgentRadius {
			continue
		}
		if d < collisionDist || (d == collisionDist && (collisionWith == "" || id < collisionWith)) {
			collisionDist = d
			collisionWith = id
		}
	}
	if collisionWith != "" {
		ag.VX, ag.VY = 0, 0
		return MoveResult{OK: false, Reason: "blocked_agent", CollisionWith: collisionWith, Distance: 0, NewState: *ag}
	}
