...
`control-workers` is the internal monorepo directory for the deterministic worker runtime of Nexus. The deployed service name remains `nexus-control-operators`. It runs four event-driven workers that consume operational events from `nexus-core` and react autonomously: detecting anomalies, coordinating incident state, applying mitigations, and verifying recovery.
...
cd control-workers && go build -o bin/nexus-control-operators ./cmd/ops-workers
```
...
# Run vet + tests
cd control-workers && go vet ./... && go test ./...
```
