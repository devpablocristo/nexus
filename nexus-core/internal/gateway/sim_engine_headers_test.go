...
func TestRun_SimEngineInternalHeaders(t *testing.T) {
	orgID := uuid.New()
	toolID := uuid.New()
	...
	svc := NewUsecases(
		...
		Config{
			...
			SimEngineBaseURL:       "http://sim-engine:8087",
			SimEngineInternalKey:   "test-internal",
	...
	if got := exec.lastHeaders["X-Sim-Engine-Internal-Key"]; got != "test-internal" {
		t.Fatalf("expected X-Sim-Engine-Internal-Key set, got %q", got)
	}
}

func TestRun_NonSimEngineDoesNotGetInternalKey(t *testing.T) {
	orgID := uuid.New()
	toolID := uuid.New()
	exec := &captureExecutor{}
	...
	if got := exec.lastHeaders["X-Sim-Engine-Internal-Key"]; got != "" {
		t.Fatalf("did not expect internal key header for non-sim-engine tool, got %q", got)
	}
}
