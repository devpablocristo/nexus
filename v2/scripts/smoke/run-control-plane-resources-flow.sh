...
READY_URL="${BASE_URL}/v1/resources"
...
create_body="$(
  curl -sS -H 'Content-Type: application/json' -X POST "${RESOURCES_URL}" -d @- <<'JSON'
{
  "type": "wallet",
...
list_body="$(curl -sS "${RESOURCES_URL}?type=wallet&environment=prod&archived=false&limit=10")"
...
archive_body="$(curl -sS -X POST "${RESOURCES_URL}/${resource_id}/archive")"
...
restore_body="$(curl -sS -X POST "${RESOURCES_URL}/${resource_id}/restore")"
...
create_policy_body="$(
  curl -sS -H 'Content-Type: application/json' -X POST "${POLICIES_URL}" -d @- <<'JSON'
{
...
policy_list_body="$(curl -sS "${POLICIES_URL}?action_type=withdrawal&resource_type=wallet")"
...
policy_archive_body="$(curl -sS -X POST "${POLICIES_URL}/${policy_id}/archive")"
...
policy_restore_body="$(curl -sS -X POST "${POLICIES_URL}/${policy_id}/restore")"
...
policy_delete_status="$(curl -sS -o /dev/null -w '%{http_code}' -X DELETE "${POLICIES_URL}/${policy_id}")"
if [[ "${policy_delete_status}" != "204" ]]; then
  echo "unexpected policy delete status: ${policy_delete_status}" >&2
  exit 1
fi

delete_status="$(curl -sS -o /dev/null -w '%{http_code}' -X DELETE "${RESOURCES_URL}/${resource_id}")"
