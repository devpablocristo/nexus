...
create_blocked_action() {
  ...
  curl -sS -H 'Content-Type: application/json' -X POST "${DATA_PLANE_URL}/v1/actions" -d @- <<JSON
{
  "action_type": "withdrawal",
...
reject_action() {
  ...
  curl -sS -H 'Content-Type: application/json' -X POST "${DATA_PLANE_URL}/v1/actions/${action_id}/reject" -d @- <<'JSON'
{
...
wait_for_http "${CONTROL_WORKERS_URL}/v1/incidents" 80 0.1
...
wait_for_http "${DATA_PLANE_URL}/v1/actions" 80 0.1
...
blocked_incidents="$(curl -sS "${CONTROL_WORKERS_URL}/v1/incidents?trigger=blocked_action&status=open")"
...
blocked_alerts="$(curl -sS "${ALERTS_URL}?channel=slack&status=pending&limit=10")"
...
rejected_incidents="$(curl -sS "${CONTROL_WORKERS_URL}/v1/incidents?trigger=approval_rejected&status=open")"
...
rejected_alerts="$(curl -sS "${ALERTS_URL}?channel=slack&status=pending&limit=10")"
