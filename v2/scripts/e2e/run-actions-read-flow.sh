...
create_action() {
  curl -sS -H 'Content-Type: application/json' -X POST "${ACTIONS_URL}" -d @- <<JSON
{
  "action_type": "withdrawal",
...
approve_action() {
  ...
  curl -sS -H 'Content-Type: application/json' -X POST "${ACTIONS_URL}/${action_id}/approve" -d @- <<'JSON'
{
...
issue_lease() {
  local action_id="$1"
  curl -sS -X POST "${ACTIONS_URL}/${action_id}/lease"
}
...
execute_action() {
  ...
  curl -sS -H 'Content-Type: application/json' -X POST "${ACTIONS_URL}/${action_id}/execute" -d @- <<JSON
{
  "lease_id": "${lease_id}",
...
list_body="$(curl -sS "${ACTIONS_URL}?action_type=withdrawal&status=pending_approval&limit=10")"
...
get_body="$(curl -sS "${ACTIONS_URL}/${action_id}")"
...
risk_body="$(curl -sS "${ACTIONS_URL}/${action_id}/risk")"
...
evidence_body="$(curl -sS "${ACTIONS_URL}/${action_id}/evidence")"
