...
if ! docker compose exec -T postgres pg_isready -U postgres >/dev/null 2>&1; then
  echo "ERROR: Postgres no responde. Desde v3/: docker compose up -d postgres" >&2
  exit 1
fi

EXISTS=$(docker compose exec -T postgres psql -U postgres -Atqc \
...
docker compose exec -T postgres psql -U postgres -c "CREATE DATABASE nexus_companion;"
