...
func (v *Verifier) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		mode := strings.TrimSpace(headers.Get("X-Nexus-Credential-Mode"))
		hasLeaseMetadata := HasLeaseMetadata(headers)
		token := TokenFromRequest(headers)
		if !hasLeaseMetadata && token == "" {
			c.Next()
			return
		}
		if token == "" {
...
func HasLeaseMetadata(headers http.Header) bool {
